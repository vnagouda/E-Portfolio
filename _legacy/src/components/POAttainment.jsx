      ...
      <div className="relative max-w-6xl mx-auto">
          ...
          {poScores.map(({ code }) => (
            <motion.button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors duration-200 focus:outline-none
                ${active === code
                  ? 'bg-[#F9B17A] border-[#F9B17A] text-white'
                  : 'bg-transparent border-[#F9B17A] text-[#F9B17A] hover:bg-[#fba96c] hover:text-black'} `}
            >
              {code}
            </motion.button>
          ))}
        ...
        <AnimatePresence exitBeforeEnter>
          <motion.div
            ...
            className="mt-6 p-6 bg-[#112240] rounded-lg border border-[#F9B17A] shadow-lg"
          >
            <h3 className="text-2xl font-semibold text-[#F9B17A] mb-2">{active}</h3>
